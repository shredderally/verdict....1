...
@media(max-width:700px){
  ...
  .sb{width:180px}.dash-main{margin-left:180px;padding:16px}
...
const CFG = {
  ...
  act_email: "activate@verdictai.com",
};
...
export default function VerdictV2() {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem("verdict_email");
    if (stored) {
      supabase.from("users").select("*").eq("email", stored).then(({ data }) => {
        if (data && data.length > 0) {
          const rec = data[0];
          setUser({ email: rec.email, track: rec.track, tier: rec.tier, runs: rec.runs || {}, firstVisit: false });
          setView("dashboard");
        }
      });
    }
  ...
  const doSignup = async () => {
    ...
    setAuthLoading(true);
    try {
      const { data: existing } = await supabase.from("users").select("email").eq("email", sf.email);
      if (existing && existing.length > 0) { setAuthErr("Account already exists."); setAuthLoading(false); return; }
      const newUser = { email: sf.email, password: sf.password, track: sf.track, tier: "free", runs: {} };
      const { error } = await supabase.from("users").insert([newUser]);
      if (error) { setAuthErr("Signup failed. Try again."); setAuthLoading(false); return; }
      const u = { email: sf.email, track: sf.track, tier: "free", runs: {}, firstVisit: true };
      localStorage.setItem("verdict_email", sf.email);
      setUser(u); setView("dashboard"); setAuthErr(""); setAuthLoading(false);
    } catch(e) { setAuthErr("Signup failed. Try again."); setAuthLoading(false); }
  };

  const doSignin = async () => {
    setAuthLoading(true);
    try {
      const { data: emailData } = await supabase.from("users").select("*").eq("email", li.email);
      const data = emailData ? emailData.filter(u => u.password === li.password) : [];
      if (!data || data.length === 0) { setAuthErr("Invalid credentials."); setAuthLoading(false); return; }
      const rec = data[0];
      localStorage.setItem("verdict_email", rec.email);
      setUser({ email: rec.email, track: rec.track, tier: rec.tier, runs: rec.runs || {}, firstVisit: false });
      setView("dashboard"); setAuthErr(""); setAuthLoading(false);
    } catch(e) { setAuthErr("Invalid credentials."); setAuthLoading(false); }
  };

  const doSignout = () => { setUser(null); localStorage.removeItem("verdict_email"); setView("landing"); };
  ...
  const runTool = useCallback(async () => {
    ...
    setUser(updUser);
    await supabase.from("users").update({ runs: newRuns }).eq("email", user.email);
    try {
  ...
  const AMOUNTS = {
    pro:   { monthly:"$89", annual:"$640.80", usdt_m:"89 USDT", usdt_a:"640.80 USDT" },
    ultra: { monthly:"$299", annual:"$2,332.20", usdt_m:"299 USDT", usdt_a:"2,332.20 USDT" },
  };
  ...
  if (view === "landing") return (
    <div className="lp">
      <nav className="nav">
        <div className="logo">Verdict<span>.</span></div>
      ...
      <div style={{textAlign:"center",padding:"20px 24px",borderTop:"1px solid var(--br)",borderBottom:"1px solid var(--br)",background:"var(--sf)"}}>
        ...
        <div style={{display:"flex",justifyContent:"center",gap:32,flexWrap:"wrap"}}>
          {[["10K+","Active users"],["45","AI tools"],["2","Specialized tracks"],["$89","Pro plan"]].map(([n,l])=>(
      ...
      <section className="sec" id="demo-sec">
        ...
        <div className="demo">
          ...
          <div className="dout"><strong>Input: "{DEMOS[demoTab].input}"</strong>{"

"}{DEMOS[demoTab].output}</div>
        </div>
      </section>
      ...
      <section className="sec" style={{paddingTop:0}}>
        ...
        <div className="how">
          {[["01","Pick your track","Creator or Business — choose once, all tools are built for your context."],
            ...
            ["03","Type and run","Describe your topic. Verdict gives you a structured, specific answer. Not a summary."]
          ].map(([n,t,d])=>(
      ...
      <section className="sec" id="pricing" style={{paddingTop:0}}>
        ...
        <div className="ptog">
          {["monthly","annual"].map(b=>(
            <button key={b} className={lpBill===b?"on":""} onClick={()=>setLpBill(b)}>{b.charAt(0).toUpperCase()+b.slice(1)}</button>
          ))}
          <span className="psave">Save 40% annually</span>
        </div>
        <div className="pgrid">
          <div className="pcard">
            <div className="p-name">Free</div>
            <div className="p-price">$0</div>
            <div className="p-billed">Forever free</div>
            <ul className="p-feats"><li>5 tools per track</li><li>3 runs per tool</li><li>No card required</li></ul>
            <button className="btn btn-out" style={{width:"100%"}} onClick={()=>{setAuthErr("");setView("signup")}}>Get started</button>
            <p className="p-who">For: testing Verdict before committing</p>
          </div>
          <div className="pcard ft">
            <div className="pbdg">Most popular</div>
            <div className="p-name">Pro</div>
            <div className="p-price">{lpBill==="monthly"?"$89":"$53.40"}<span>/mo</span></div>
            <div className="p-billed">{lpBill==="annual"?"$640.80/year — save $427.20":"Billed monthly"}</div>
          ...
          <div className="pcard uc">
            <div className="p-name">Ultra</div>
            <div className="p-price">{lpBill==="monthly"?"$299":"$194.35"}<span>/mo</span></div>
            <div className="p-billed">{lpBill==="annual"?"$2,332.20/year — save $1,255.80":"Billed monthly"}</div>
...
CONFIDENCE: 81% viable. Strongest opportunity in anglophone West Africa.`},
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,border:"1px solid var(--br)",borderRadius:"var(--rl)",overflow:"hidden"}}>
          <thead>
            <tr style={{background:"var(--sf)"}}>
              ...
              <th style={{padding:"12px 16px",textAlign:"left",color:"var(--gd)",fontWeight:500,fontSize:11,letterSpacing:".05em",textTransform:"uppercase",borderBottom:"1px solid var(--br)"}}>Verdict</th>
            </tr>
          </thead>
          <tbody>
            {[
      ...
      <section className="sec" style={{paddingTop:0}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:16,marginTop:48}}>
          {[
            ...
            {name:"Diego R.",location:"São Paulo, Brazil 🇧🇷",role:"Creator · 67K subs",text:"Tried every tool on the market. Verdict is the only one where I feel like I'm talking to someone who actually understands strategy."},
            {name:"Amara N.",location:"Johannesburg, SA 🇿🇦",role:"Business consultant",text:"The GTM Planner gave my client a 90-day launch plan in one session. I charged them $800 for a deck built in 2 hours with Verdict."},
            ...
            {name:"Yuki T.",location:"Tokyo, Japan 🇯🇵",role:"Creator · 112K subs",text:"The Viral Blueprint gave me a framework I've used for 8 videos in a row. 6 hit over 100K views. Verdict is now non-negotiable in my workflow."},
      ...
      <footer className="foot">
        <div className="foot-logo">Verdict<span>.</span> <span style={{fontSize:10,color:"var(--txd)",fontFamily:"var(--fm)",marginLeft:8}}>by ApexWithin</span></div>
        <div className="foot-r">© 2026 ApexWithin · verdictai.com</div>
      </footer>
    </div>
  );
  ...
  if (view === "signin") return (
    <div className="auth-pg">
      <div className="auth-c">
        <div className="auth-logo">Verdict<span>.</span></div>
        ...
        <div className="af"><label>Password</label><input type="password" value={li.password} onChange={e=>setLi(p=>({...p,password:e.target.value}))} placeholder="••••••••" onKeyDown={e=>e.key==="Enter"&&doSignin()} /></div>
  ...
  if (view === "signup") return (
    <div className="auth-pg">
      <div className="auth-c">
        ...
        <div className="af"><label>Password</label><input type="password" value={sf.password} onChange={e=>setSf(p=>({...p,password:e.target.value}))} placeholder="6+ characters" /></div>
        <div className="af">
  ...
  return (
    <div className="dash">
      <aside className="sb">
        <div className="sb-logo">Verdict<span>.</span></div>
        <div className="sb-trk">{user.track} · <span style={{color:user.tier==="ultra"?"#a07535":user.tier==="pro"?"var(--gd)":"var(--ok)"}}>{user.tier}</span></div>
      ...
      <main className="dash-main">
        ...
        {user.firstVisit && !onbDis && (
          <div className="onb">
            <div className="onb-t"><strong>Welcome to Verdict.</strong> You're on the {user.track} track. Pick any free tool below and press Run.</div>
      ...
      {upgradeOpen && (
        <div className="ov" onClick={e=>e.target===e.currentTarget&&setUpgradeOpen(false)}>
          <div className="modal">
            <div className="mh">
              <div className="mt">Upgrade Verdict</div>
            ...
            <div className="mb">
              <div style={{display:"flex",gap:8,marginBottom:20,alignItems:"center"}}>
                ...
                <span className="psave">Annual saves 40%</span>
              </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
                {["pro","ultra"].map(p=>(
                  <div key={p} onClick={()=>setUpgPlan(p)}
                    ...
                    <div style={{fontFamily:"var(--fm)",fontSize:20}}>{AMOUNTS[p][billing==="monthly"?"monthly":"annual"]}<span style={{fontSize:11,color:"var(--txs)"}}>{billing==="annual"?"/yr":"/mo"}</span></div>
                    <div style={{fontSize:10,color:"var(--txs)",marginTop:3}}>{p==="pro"?"80 runs/mo · 11–13 tools":"Unlimited · all tools"}</div>
                  </div>
                ))}
              </div>
                ...
                <div className="bnb">
                  ...
                  <div className="bnb-steps">
                    {[
                      ...
                      ["2",`Enter the Verdict Pay ID below`],
                      ["3",`Send exactly ${AMOUNTS[upgPlan][billing==="monthly"?"usdt_m":"usdt_a"]} in USDT`],
                      ["4",`Email reference + your Verdict email to ${CFG.act_email}`],
